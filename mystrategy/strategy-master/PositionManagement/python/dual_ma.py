...
class DualMA(StrategyBase):
    ...
    def on_execrpt(self, execution):
        ...
        if execution.sec_id != self.sec_id:
            return
        self.logger.info("received execution: exec_type = {0}, rej_reason = {1} ".format(execution.exec_type,
                                                                                         execution.ord_rej_reason_detail))
    ...
    def on_order_partially_filled(self, order):
        ...
        self.logger.info('''
          ...
          '''.format(order.sec_id, order.side, order.position_effect, order.volume, order.filled_vwap,
                     order.filled_volume))
    ...
    def clean_final_orders(self, order):
        self.logger.info(
            "try to remove finished order {0}, sec_id {1} volume {2} price {3} ".format(order.cl_ord_id, order.sec_id,
    ...
    def cancel_unfinished_orders(self):
        if len(self.orders) > 0:
            for o in self.orders:
                self.logger.info(
                    "try to cancel order {0}, sec_id {1} volume {2} price {3} ".format(o.cl_ord_id, o.sec_id, o.volume,
                                                                                       o.price))
                # self.trade_count -= 1
    ...
    def algo_action(self):
        ...
        self.logger.info('short ma: {0}, long ma: {1}, life line: {2}'.format(round(sma[-1], 4), round(lma[-1], 4),
                                                                              round(life[-1], 4)))
        self.logger.info(
            'short long ma delta: {0}, last_ma: {1}, momentum: {2}'.format(round(s_l_ma_delta, 4), round(last_ma, 4),
                                                                           round(momentum, 4)))
        self.logger.info(
            '## short ma moving long: {0}, moving short: {1}; Trends is long: {2}, is short: {3}.'.format(moving_long,
                                                                                                          moving_short,
                                                                                                          long_trends,
                                                                                                          short_trends))
        ...
        price_base = max(self.last_price, close[-1]) if long_trends else (
            min(self.last_price, close[-1]) if short_trends else self.last_price)
        ...
        if long_trends:  ## short ma cross up long ma
            ...
            if (a_p is None or a_p.volume < eps):
                signal_filter = (
                                            moving_long or momentum >= significant_diff) and s_l_ma_delta > threshold and self.trade_count < self.trade_limit
            ...
            ## 有多仓，且有止赢要求
                ...
                if self.positive_stop and (checking_limit or b_pnl >= stop_profit_threshold):
                    ord_price = price_base - self.hops * self.tick_size
                    self.logger.info(
                        "trade count {0} to limit time[{1}], stop profit, close long @ {2}".format(self.trade_count,
                                                                                                   self.trade_limit,
                                                                                                   ord_price))
        ...
        ## 空信号
            ...
            if (b_p is None or b_p.volume < eps):
                signal_filter = (
                                            moving_short or momentum <= -significant_diff) and s_l_ma_delta < -threshold and self.trade_count < self.trade_limit
                vol = self.trade_unit[self.trade_count]  ## get order volume from configured list
                if signal_filter and vol > eps:
                    ord_price = price_base - self.hops * self.tick_size
            ...
            ## 有空仓，且有止赢要求
                ...
                if self.positive_stop and (checking_limit or a_pnl >= stop_profit_threshold):
                    ord_price = price_base + self.hops * self.tick_size
                    self.logger.info(
                        "trade count {0} to limit time [{1}], stop profit, close short @ {2}".format(self.trade_count,
                                                                                                     self.trade_limit,
                                                                                                     ord_price))
