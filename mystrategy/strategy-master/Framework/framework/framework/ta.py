...
class TAStrategy(StrategyBase, TAMixin, PositionMixin, OrderMixin, VolumeMixin):
    ...
    def on_execrpt(self, execution):
        self.logger.debug(
            "received execution: exec_type = {}, detail: {}".format(execution.exec_type, to_dict(execution)))
    ...
    def on_order_cancelled(self, order):
        ...
        '''.format(order.cl_ord_id, order.sec_id, order.side, order.position_effect, order.volume, order.price,
                   order.filled_volume))
    ...
    def on_order_cancel_rejected(self, execrpt):
        ...
        '''.format(execrpt.cl_ord_id, execrpt.sec_id, execrpt.side, execrpt.position_effect, execrpt.price,
                   execrpt.volume))
    ...
    def on_order_rejected(self, execrpt):
        ...
        '''.format(execrpt.cl_ord_id, execrpt.sec_id, execrpt.side, execrpt.position_effect, execrpt.volume,
                   execrpt.price, execrpt.ord_rej_reason_detail))
    ...
    def open_long(self, exchange, sec_id, price, volume):
        ...
        if not self.long_trend_filtering(symbol):
            self.logger.info(
                "trend filtered, now {} trend is short, ignore open long order {}@{:.2f}".format(symbol, volume, price))
            return
    ...
    def open_short(self, exchange, sec_id, price, volume):
        ...
        if not self.short_trend_filtering(symbol):
            self.logger.info(
                "trend filtered, now {} trend is long, ignore open short order {}@{:.2f}".format(symbol, volume, price))
            return
