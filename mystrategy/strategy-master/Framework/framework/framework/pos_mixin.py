...
class PositionMixin(Context, object):
    ...
    def close_long_position(self, b_p, ord_price=None, msg=None, sync=False):
        ...
        price = ord_price if ord_price is not None else helper.aggressive_oppsite_price(last_tick, OrderSide_Ask,
                                                                                        self.hops, tick_size)
        ...
        self.logger.info(
            "try to close long {} ... vol {} @ prc {:.3f}, if future today's position first, msg = {}".format(symbol,
                                                                                                              b_p.volume,
                                                                                                              price,
                                                                                                              msg))
    ...
    def close_short_position(self, a_p, ord_price=None, msg=None, sync=False):
        ...
        # side = OrderSide_Bid
        # price = ord_price if ord_price else last_price + self.hops * tick_size
        price = ord_price if ord_price is not None else helper.aggressive_oppsite_price(last_tick, OrderSide_Bid,
                                                                                        self.hops, tick_size)
        ...
        self.logger.info(
            "try to close short {} ... vol {} @ {:.3f}, if future today's position first, msg = {}".format(symbol,
                                                                                                           a_p.volume,
                                                                                                           price, msg))
    ...
    def get_highest_lowest_price_since_open(self, sym, init_time):
        ...
        if (exchange in ('SHSE', 'SZSE', 'CFFEX') and (
                15 > now.datetime.hour > 9 or (now.datetime.hour == 9 and now.datetime.minute > 30))) or \
                (exchange in ('DCE', 'CZCE', 'SHFE') and (
                        now.datetime.hour in (21, 22, 23, 0, 1, 2) or 15 > now.datetime.hour >= 9)):
    ...
    def close_old_positions(self, tick, min_pnl, minutes=1):
        ...
        if a_p and a_p.available > 0:
            ...
            self.logger.debug(
                "now = {}, create time = {}, time span {} seconds".format(now, create_time, time_span.total_seconds()))
            a_pnl = (a_p.vwap - price) / a_p.vwap / mr
            if time_span.total_seconds() > minutes * ONE_MINUTE and a_pnl < min_pnl:
                msg = 'Non-profit old position, short {} (pnl = {:.2f}%) aged over {:.4f} > {} minutes'.format(symbol,
                                                                                                               a_pnl * 100.0,
                                                                                                               time_span.total_seconds() / 60.0,
                                                                                                               minutes)
            ...
            if self.follow_trends and symbol in self.trends:
                ...
                if trend.elapsed_seconds_since_stopped(a_p.side, now) > minutes * ONE_MINUTE:
                    msg = '{} short (pnl = {:.2f}%), trend {} stopped over {} minutes'.format(symbol, a_pnl * 100.0,
                                                                                              trend, minutes)
                    self.close_short_position(a_p, msg=msg)
                    self.clear_trade_counter(symbol, OrderSide_Ask)
        ...
        if b_p and b_p.available > 0:
            ...
            if time_span.total_seconds() > minutes * ONE_MINUTE and b_pnl < min_pnl:
                msg = 'Non-profit old position, long {} (pnl = {:.2f}%) aged over {:.4f} > {} minutes'.format(symbol,
                                                                                                              b_pnl * 100.0,
                                                                                                              time_span.total_seconds() / 60.0,
                                                                                                              minutes)
            ...
            if self.follow_trends and symbol in self.trends:
                ...
                if trend.elapsed_seconds_since_stopped(b_p.side, now) > minutes * ONE_MINUTE:
                    msg = '{} long (pnl = {:.2f}%), trend {} stopped over {} minutes'.format(symbol, b_pnl * 100.0,
                                                                                             trend, minutes)
                    self.close_long_position(b_p, msg=msg)
                    self.clear_trade_counter(symbol, OrderSide_Bid)
    ...
    def care_positions_for_symbol(self, symbol, a_p, b_p, highest_price=None, lowest_price=None, act=True):
        ...
        self.logger.debug(
            "{} pos long: {} vwap: {:.2f}, pos short: {}, vwap: {:.2f}".format(symbol, b_p.volume if b_p else 0.0,
    ...
    def try_stop_action(self, symbol, a_p, b_p):
        ...
        self.logger.debug(
            "{} trends {}, short volume {}, long volume {}".format(symbol, trends, a_p.volume if a_p else 0,
                                                                   b_p.volume if b_p else 0))
        ...
        if b_p and b_p.volume > 0 and b_p.available > 0:
            ...
            self.logger.debug("long {} current pnl = {:.2f}%, highest pnl = {:.2f}%".format(symbol, b_pnl * 100.0,
                                                                                            highest_pnl * 100.0))

            # 触发止损阈值，或趋势不再继续
            if self.stop_lose and b_pnl < -stop_lose_threshold:
                msg = "lose threshold reached ({:.2f}% <= -{:.2f}%), stop lose, close long...".format(b_pnl * 100.0,
                                                                                                      stop_lose_threshold * 100.0)
            ...
            if self.positive_stop and threshold < b_pnl <= (1 - self.drawdown) * highest_pnl:
                msg = "pnl draw down (pnl = {:.2f}%, highest pnl = {:.2f}%), stop profit, close long...".format(
                    b_pnl * 100.0, highest_pnl * 100.0)
                # ord_price = price_base - self.hops * tick_size
                ord_price = helper.aggressive_oppsite_price(last_tick, OrderSide_Ask, self.hops, tick_size)
                self.close_long_position(b_p, ord_price=ord_price, msg=msg)
                self.clear_trade_counter(symbol, OrderSide_Bid)
            ...
            if self.positive_stop and b_pnl >= stop_profit_threshold:
                # ord_price = price_base + self.hops * tick_size
                ord_price = helper.aggressive_oppsite_price(last_tick, OrderSide_Ask, -self.hops, tick_size)
                msg = "take fixed earning ({:.2f}% >= {:.2f}%), stop profit, close long...".format(b_pnl * 100.0,
                                                                                                   stop_profit_threshold * 100.0)
                self.close_long_position(b_p, ord_price=ord_price, msg=msg)
                self.clear_trade_counter(symbol, OrderSide_Bid)
        ...
        if a_p and a_p.volume > 0 and a_p.available > 0:
            ...
            self.logger.debug("short {} current pnl = {:.2f}%, highest pnl = {:.2f}%".format(symbol, a_pnl * 100.0,
                                                                                             highest_pnl * 100.0))
            # 触发止损阈值
            if self.stop_lose and a_pnl < - stop_lose_threshold:
                msg = "lose threshold reached ({:.2f}% <= -{:.2f}%), stop lose, close short...".format(a_pnl * 100.0,
                                                                                                       stop_lose_threshold * 100.0)
                # ord_price = price_base + self.hops * tick_size
            ...
            if self.positive_stop and threshold < a_pnl <= (1 - self.drawdown) * highest_pnl:
                msg = "pnl draw down (pnl = {:.2f}%, highest pnl = {:.2f}%), stop profit, close short...".format(
                    a_pnl * 100.0, highest_pnl * 100.0)
                # ord_price = price_base + self.hops * tick_size
                ord_price = helper.aggressive_oppsite_price(last_tick, OrderSide_Bid, self.hops, tick_size)
                self.close_short_position(a_p, ord_price=ord_price, msg=msg)
                self.clear_trade_counter(symbol, OrderSide_Ask)
            ...
            if self.positive_stop and a_pnl >= stop_profit_threshold:
                msg = "take fixed earning ({:.2f}% >= {:.2f}%), stop profit, close short...".format(a_pnl * 100.0,
                                                                                                    stop_profit_threshold * 100.0)
                # ord_price = price_base - self.hops * tick_size
                ord_price = helper.aggressive_oppsite_price(last_tick, OrderSide_Bid, -self.hops, tick_size)
                self.close_short_position(a_p, ord_price=ord_price, msg=msg)
    ...
    def ma_trends(self, close, last_price, momentum_side=0):
        ...
        self.logger.debug(
            'short ma: {:.4f}, long ma: {:.4f}, life line: {:.4f}, len of close = {}'.format(sma[-1], lma[-1], life[-1],
                                                                                             len(close)))
        self.logger.debug(
    ...
    def calc_vol(self, bar, side):
        ...
        if cash.available - amount < cash.nav * (1.0 - self.high_position_ratio):
            self.logger.info("{} {}.{} aborted, cash left will be {:.2f}, may exceed position limit {:.2f}%"
                             .format(act, bar.exchange, bar.sec_id, cash.available - amount,
                                     self.high_position_ratio * 100.0))
            return 0
