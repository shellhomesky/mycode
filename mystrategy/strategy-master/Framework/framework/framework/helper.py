...
def close_long_position(inst, b_p, price=0.0, sync=False):
    if b_p.exchange in (u'SHSE', u'SZSE'):  ## stocks
        if b_p.available_yesterday:
            inst.logger.info(
                "close long yesterday {}.{} {} @ {:.3f}".format(b_p.exchange, b_p.sec_id, b_p.available_yesterday,
                                                                price))
            if not sync:
        ...
        if b_p.exchange == u'SHFE':  ## special in SHFE
            if b_p.available_today > 0:
                inst.logger.info(
                    "close long today {}.{} {} @ {:.3f}".format(b_p.exchange, b_p.sec_id, b_p.available_today, price))
                if not sync:
            ...
            if b_p.available > 0:
                ...
                inst.logger.error(
                    "Error: {}.{} available {} not a positive number".format(b_p.exchange, b_p.sec_id, b_p.available))
...
def close_short_position(inst, a_p, price=0.0, sync=False):
    if a_p.exchange in (u'SHSE', u'SZSE'):  ## stocks, something must be wrong
        if a_p.available_yesterday:  # support short stocks now
            inst.logger.info(
                "close short yesterday {}.{} {} @ {:.3f}".format(a_p.exchange, a_p.sec_id, a_p.available_yesterday,
                                                                 price))
            if not sync:
        ...
        if a_p.exchange == u'SHFE':  ## special in SHFE
            if a_p.available_today > 0:
                inst.logger.info(
                    "close short today {}.{} {} @ {:.3f}".format(a_p.exchange, a_p.sec_id, a_p.available_today, price))
                if not sync:
            ...
            if a_p.available > 0:
                ...
                inst.logger.error(
                    "Error: {}.{} available {} not a positive number".format(a_p.exchange, a_p.sec_id, a_p.available))
