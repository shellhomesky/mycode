...
class Strategy(StrategyBase):
    ...
    def md_init(self):
        ...
        symbol_list1 = set(instrument.symbol for instrument in instruments2 + instruments1 if
                           instrument.symbol[5] not in ['2', '9'])  # 获取当日可交易的股票，剔除B股
        constituents = self.get_constituents('SHSE.000985')
        symbol_list2 = set(
            constituent.symbol for constituent in constituents)  # 获取中证全指成分股（剔除ST、*ST股票，以及上市时间不足3个月等股票后剩余的股票）
    ...
    def on_bar(self, bar):
        ...
        if len(positions) == 0:
            ...
            for b in self.buy_dict.values():
                order = self.open_long(b.exchange, b.sec_id, 0,
                                       int(cash.available * 0.95 / len(self.buy_dict) / b.close / 100) * 100)
            return
        # endregion
        ...
        if order.sec_id in self.sell_dict and order.strategy_id == self.strategy_id:
            ...
            if len(self.sell_dict) == 0:  # 由于资金每次都开满，等卖盘全部成交资金回流时再买入
                ...
                for bar in self.buy_dict.values():
                    self.open_long(bar.exchange, bar.sec_id, 0,
                                   int(cash.available * 0.95 / len(self.buy_dict) / bar.close / 100) * 100)
